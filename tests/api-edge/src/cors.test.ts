...
const stageEnv: Env = { ENVIRONMENT: "stage", CONSOLE_CUSTOM_DOMAIN: "stage.jjhj.dev" };
const prodEnv: Env = { ENVIRONMENT: "prod", CONSOLE_CUSTOM_DOMAIN: "prod.jjhj.dev" };
...
describe("api-edge cors", () => {
  describe("isAllowedOrigin — stage environment", () => {
    it("allows the stage custom domain origin", () => {
      expect(isAllowedOrigin("https://stage.jjhj.dev", stageEnv)).toBe(true);
    });
    ...
    it("rejects the prod custom domain origin", () => {
      expect(isAllowedOrigin("https://prod.jjhj.dev", stageEnv)).toBe(false);
    });
    ...
    it("rejects the legacy Pages console origins", () => {
      expect(isAllowedOrigin("https://jjhj-web-console.pages.dev", stageEnv)).toBe(false);
      expect(isAllowedOrigin("https://jjhj-web-console-stage.pages.dev", stageEnv)).toBe(false);
      expect(isAllowedOrigin("https://jjhj-web-console-prod.pages.dev", stageEnv)).toBe(false);
      expect(isAllowedOrigin("https://abc123.jjhj-web-console-stage.pages.dev", stageEnv)).toBe(false);
    });
  ...
  describe("isAllowedOrigin — prod environment", () => {
    it("allows the prod custom domain origin", () => {
      expect(isAllowedOrigin("https://prod.jjhj.dev", prodEnv)).toBe(true);
    });
    ...
    it("rejects the stage custom domain origin", () => {
      expect(isAllowedOrigin("https://stage.jjhj.dev", prodEnv)).toBe(false);
    });
    ...
    it("rejects the legacy Pages console origins", () => {
      expect(isAllowedOrigin("https://jjhj-web-console.pages.dev", prodEnv)).toBe(false);
      expect(isAllowedOrigin("https://jjhj-web-console-prod.pages.dev", prodEnv)).toBe(false);
      expect(isAllowedOrigin("https://feat-branch-42.jjhj-web-console-stage.pages.dev", prodEnv)).toBe(false);
    });
  ...
  describe("isAllowedOrigin — fallback (test/unknown environment)", () => {
    it("rejects custom domain origins when CONSOLE_CUSTOM_DOMAIN is not set", () => {
      expect(isAllowedOrigin("https://stage.jjhj.dev", testEnv)).toBe(false);
      expect(isAllowedOrigin("https://prod.jjhj.dev", testEnv)).toBe(false);
    });
    ...
    it("rejects legacy Pages console origins in fallback env", () => {
      expect(isAllowedOrigin("https://jjhj-web-console-stage.pages.dev", testEnv)).toBe(false);
      expect(isAllowedOrigin("https://jjhj-web-console-prod.pages.dev", testEnv)).toBe(false);
    });
  ...
  describe("handlePreflight", () => {
    ...
    it("returns 204 without CORS headers when prod custom domain hits stage API", () => {
      const req = new Request("https://api.test/v1/auth/session", {
        method: "OPTIONS",
        headers: { origin: "https://prod.jjhj.dev" },
      });
      const res = handlePreflight(req, stageEnv);
      expect(res).not.toBeNull();
      expect(res!.status).toBe(204);
