...
describe("resolveConfigDir", () => {
  it("respects JJHJ_CONFIG_DIR override", () => {
    const prev = process.env["JJHJ_CONFIG_DIR"];
    process.env["JJHJ_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["JJHJ_CONFIG_DIR"];
      else process.env["JJHJ_CONFIG_DIR"] = prev;
    }
  });
});
