...
`jjhj` — first-class TypeScript CLI for the jjhj control
...
This package is internal to the `jjhj` monorepo and is not
...
```
jjhj login    [--api-url=URL] [--token=BEARER]
jjhj logout
jjhj whoami
```
...
```
jjhj org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
jjhj project create <name> [--idempotency-key=KEY]
jjhj env create <project-id> <name> [--idempotency-key=KEY]
jjhj api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
jjhj webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
jjhj usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
jjhj billing summary
jjhj audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`jjhj org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `jjhj login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/jjhj/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/jjhj/config.json` (mode 0644, not a secret). Override
both via `JJHJ_CONFIG_DIR` (used by tests).
