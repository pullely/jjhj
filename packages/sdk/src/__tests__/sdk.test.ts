...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new jjhj({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => jjhjError]> = [
  ...
  it("forward-compat: unknown error code → base jjhjError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(jjhjError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as jjhjError).code).toBe("future_quota_exceeded");
    }
  });
