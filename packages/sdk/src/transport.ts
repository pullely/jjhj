// Base HTTP transport for the jjhj SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `jjhjError`
...
export interface ClientOptions {
  /**
   * Base URL of the jjhj api-edge worker, e.g.
   * `https://api.jjhj.dev`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `jjhj` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "jjhj SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "jjhj SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
