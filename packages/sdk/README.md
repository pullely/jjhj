...
jjhj TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the jjhj control plane API.

## Install (workspace)

This package is internal to the `jjhj` monorepo. Add it as a
...
```ts
import { jjhj } from "@saas/sdk";

const client = new jjhj({
  baseUrl: "https://api.jjhj.dev",
  auth: { kind: "bearer", token: process.env["JJHJ_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `jjhjError`. Branch on
...
| `jjhjError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `jjhjError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new jjhj({ fetch })`.
