...
Your backend holds one jjhj API key and **zero GitHub credentials**.
...
const jjhj = new jjhj({
  baseUrl: process.env.JJHJ_API_URL!,
  token: process.env.JJHJ_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await jjhj.integrations.issueGithubToken(ORG_ID, {
